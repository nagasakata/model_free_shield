...
class EvalCallback(callbacks.EvalCallback):
    ...
    def __init__(self,
        ...
        super(EvalCallback, self).__init__(eval_env = eval_env, 
...
def run(shield, 
        ...
        callback, 
        shield_life, 
        use_shield_while_learning : bool, 
        use_shield_while_evaluation : bool,
        times : int,
        ep_length : int):
    
    if use_shield_while_learning:
        ...
        if use_shield_while_evaluation:
            label = f'Shield-learnng-True-evaluation-True'
            eval_env = shield(env=game, ltl_formula=ltl_formula).get_wrapper()
        else:
            label = f'Shield-learnng-True-evaluation-False'
            eval_env = None
        ...
        if use_shield_while_evaluation:
            label = f'None-learnng-False-evaluation-True'
            eval_env = shield(env=game, ltl_formula=ltl_formula).get_wrapper()
        else:
            label = f'None-learnng-False-evaluation-False'
            eval_env = None
...
def train(env, label, game, callback, total_steps=int(5e5), learning_rate=1e-4, policy='MlpPolicy', root_dir='.',
    ...
    model = MYPPO(policy, env, verbose=1, tensorboard_log=log_dir, learning_rate=learning_rate)
    # Shielding the policy for preemptive shielding
    if isinstance(env, ModelFreeShieldWrapper):
        shield_policy(model, eval_env=env)
